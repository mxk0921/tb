package tb;

import com.alibaba.fastjson2.JSONException;
import com.alibaba.fastjson2.JSONFactory;
import com.alibaba.fastjson2.JSONReader;
import java.lang.reflect.Type;
import java.util.Arrays;
import java.util.Collection;

/* compiled from: Taobao */
/* loaded from: /Users/corson/Downloads/taobao_decompiled/build/apk/classes2.dex */
public class cfk extends xgk {
    public static final cfk b = new cfk(null);
    public static final long c = ls9.a("[B");

    public cfk(String str) {
        super(byte[].class);
    }

    @Override // tb.xgk, tb.jdk
    public Object i(Collection collection) {
        byte b2;
        byte[] bArr = new byte[collection.size()];
        int i = 0;
        for (Object obj : collection) {
            if (obj == null) {
                b2 = 0;
            } else if (obj instanceof Number) {
                b2 = ((Number) obj).byteValue();
            } else {
                k2a M = JSONFactory.defaultObjectReaderProvider.M(obj.getClass(), Byte.TYPE);
                if (M != null) {
                    b2 = ((Byte) M.apply(obj)).byteValue();
                } else {
                    throw new JSONException("can not cast to byte " + obj.getClass());
                }
            }
            bArr[i] = b2;
            i++;
        }
        return bArr;
    }

    @Override // tb.jdk
    public Object o(JSONReader jSONReader, Type type, Object obj, long j) {
        if (jSONReader.n2()) {
            return null;
        }
        if (jSONReader.v1()) {
            byte[] bArr = new byte[16];
            int i = 0;
            while (!jSONReader.u1()) {
                if (!jSONReader.e1()) {
                    int i2 = i + 1;
                    if (i2 - bArr.length > 0) {
                        int length = bArr.length;
                        int i3 = length + (length >> 1);
                        if (i3 - i2 < 0) {
                            i3 = i2;
                        }
                        bArr = Arrays.copyOf(bArr, i3);
                    }
                    bArr[i] = (byte) jSONReader.p2();
                    i = i2;
                } else {
                    throw new JSONException(jSONReader.a1("input end"));
                }
            }
            jSONReader.w1();
            return Arrays.copyOf(bArr, i);
        } else if (!jSONReader.l1()) {
            throw new JSONException(jSONReader.a1("TODO"));
        } else if ((jSONReader.B0(j) & JSONReader.Feature.Base64StringAsByteArray.mask) != 0) {
            return b2d.c(jSONReader.Q2());
        } else {
            return jSONReader.Y1();
        }
    }

    @Override // tb.jdk
    public Object q(JSONReader jSONReader, Type type, Object obj, long j) {
        if (jSONReader.y1((byte) -110)) {
            long T2 = jSONReader.T2();
            if (!(T2 == c || T2 == bfk.d)) {
                throw new JSONException("not support autoType : " + jSONReader.U0());
            }
        }
        if (jSONReader.c1()) {
            return jSONReader.Y1();
        }
        if (jSONReader.l1()) {
            return b2d.c(jSONReader.Q2());
        }
        int c3 = jSONReader.c3();
        if (c3 == -1) {
            return null;
        }
        byte[] bArr = new byte[c3];
        for (int i = 0; i < c3; i++) {
            bArr[i] = (byte) jSONReader.p2();
        }
        return bArr;
    }
}

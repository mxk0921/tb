package com.ali.user.mobile.login.action;

import tb.t2o;

/* compiled from: Taobao */
/* loaded from: /Users/corson/Downloads/taobao_decompiled/build/apk/classes2.dex */
public class LoginResActions {
    public static final String LOGIN_CANCEL_ACTION = "com.ali.user.sdk.login.CANCEL";
    public static final String LOGIN_CLOSE_ACTION = "com.ali.user.sdk.login.CLOSE";
    public static final String LOGIN_FAIL_ACTION = "com.ali.user.sdk.login.FAIL";
    public static final String LOGIN_NETWORK_ERROR = "com.ali.user.sdk.login.NETWORK_ERROR";
    public static final String LOGIN_OPEN_ACTION = "com.ali.user.sdk.login.OPEN";
    public static final String LOGIN_SUCCESS_ACTION = "com.ali.user.sdk.login.SUCCESS";
    public static final String REG_CANCEL = "com.ali.user.sdk.register.cancel";
    public static final String WEB_ACTIVITY_CANCEL = "com.ali.user.sdk.webview.cancel";
    public static final String WEB_ACTIVITY_RESULT = "com.ali.user.sdk.webview.result";

    /* compiled from: Taobao */
    /* loaded from: /Users/corson/Downloads/taobao_decompiled/build/apk/classes2.dex */
    public static class LoginFailCode {
        public static final int ALIPAY_LOGIN_EXCEPTION = 712;
        public static final int AUTO_LOGIN_LIMIT = 728;
        public static final int LOGIN_DATA_EXCEPTION = 711;
        public static final int UCC_LOGIN_EXCEPTION = 731;
        public static final int UNIFY_TOKEN_LOGIN_EXCEPTION = 714;

        static {
            t2o.a(69206113);
        }
    }

    static {
        t2o.a(69206112);
    }
}
